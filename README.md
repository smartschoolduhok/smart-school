...
- Phase 21D منفذة على `codex/phase-21d-homework` ومفتوحة للمراجعة في [Draft PR #52](https://github.com/smartschoolduhok/smart-school/pull/52): أُنشئ R2 الخاص `smart-school-homework-staging` وربط بـPreview فقط، وطُبقت `0041_homework.sql` وحدها على STAGING فأصبح السجل `42/42` بلا pending وبعدد `83/81` جدولًا وFK نظيف. يفرض التنفيذ حارسًا يفشل مغلقًا عند `1,000,000,000` بايت ويصالح كل مفاتيح R2 مع metadata؛ نجحت الاختبارات (`20/20` و`1598/1598`) والـPreview وQuality Gates. توقف QA المصادق في المدرسة 2 لأن publish وwithdraw التزما في D1 لكن أعادا `409 homework_stale`؛ عُطلت هويات وروابط QA دون حذف صفوف الأعمال أو التدقيق، وتطابقت بيانات المدرستين 1 و3 في 62 جدولًا محميًا. لم تُمس Production ولم يحدث merge أو نشر يدوي. [عقد التنفيذ والأدلة](docs/PHASE_21D_HOMEWORK_QA.md) و[سجل بوابة STAGING](docs/PHASE_21D_STAGING_GATE.md).
