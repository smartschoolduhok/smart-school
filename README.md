...
- Phase 21D منفذة محليًا على `codex/phase-21d-homework` ومفتوحة للمراجعة في [Draft PR #52](https://github.com/smartschoolduhok/smart-school/pull/52): تربط الواجب بالتكليف التدريسي الفعلي وتعيد التحقق من موضع المادة والصف/الشعبة عند الإنشاء والنشر، وتفصل المسودة عن النشر والسحب والنسخة المصححة، وتثبّت جمهور الطلاب ذريًا، وتعرض لولي الأمر DTO محدودًا، وتوفر مرفقات JPEG/PNG/WebP/PDF محمية مع تنظيف R2 قابل لإعادة المحاولة. تضيف Migration `0041` خمسة جداول؛ fresh Local D1 عند `42/42` و`83` جدولًا محسوبًا (`81` جدول تطبيق) مع FK نظيف. نجحت اختبارات الوحدة `17/17` ومصفوفة الانحدار `1595/1595`. لم يُنشأ R2 بعيد، ولم تُطبّق `0041` على STAGING، ولم يحدث وصول إلى Production. [عقد التنفيذ والأدلة المحلية](docs/PHASE_21D_HOMEWORK_QA.md) و[بوابة STAGING المجهزة](docs/PHASE_21D_STAGING_GATE.md).
