...
- Phase 21D منفذة على `codex/phase-21d-homework` ومفتوحة للمراجعة في [Draft PR #52](https://github.com/smartschoolduhok/smart-school/pull/52): تربط الواجب بالتكليف التدريسي الفعلي وتوفر مرفقات محمية ودورة مسودة/نشر/سحب/تصحيح. نجحت الاختبارات المحلية (`17/17` و`1595/1595`). في بوابة STAGING اجتاز preflight والنسخ والاستعادة المحلية وتمرين `0041` وفحص عدم الـdrift: STAGING بقيت عند `41` migration و`78/76` جدولًا مع `0041` وحدها pending، بينما وصل التمرين المحلي إلى `42` و`83/81` مع FK نظيف وتطابق البيانات التاريخية. توقفت البوابة قبل أول كتابة بعيدة لأن R2 غير مفعّل على حساب Cloudflare (`10042`)؛ لذلك لم يُنشأ bucket، ولم تُطبّق `0041` على STAGING، ولم يُضف binding، ولم يبدأ QA، ولم يحدث وصول إلى Production أو merge. [عقد التنفيذ والأدلة](docs/PHASE_21D_HOMEWORK_QA.md) و[سجل بوابة STAGING](docs/PHASE_21D_STAGING_GATE.md).
