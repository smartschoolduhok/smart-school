...
المصدر المعتمد للترتيب هو مجلد `migrations/`. يوجد حاليًا 42 ملفًا حتى `0041`، مع وجود ملفين تاريخيين يحملان بادئة `0014` ويُطبّقان بترتيب الاسم الكامل. عند إغلاق Phase 21C كانت STAGING عند `41/41` حتى `0040`؛ هذه حالة تاريخية يجب التحقق منها مجددًا قبل أي تنفيذ. وجود الملف في Git لا يعني أنه طُبّق على STAGING أو Production؛ تحقّق من السجل والقائمة المعلّقة قبل أي كتابة بعيدة.
...
| `0041_homework.sql` | الواجبات والجمهور والمرفقات والتدقيق وحراسة دورة النشر؛ محلي/PR فقط حتى التفويض البعيد |
