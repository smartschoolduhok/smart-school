...
const workRoot = mkdtempSync(join(tmpdir(), 'smart-school-phase20d-staging-local-'));
...
const databaseName = 'smart-school-phase20d-rehearsal-local-only';
...
runLocal(['migrations', 'apply', databaseName], 'apply-0034');
const pendingOutput = runLocal(['migrations', 'list', databaseName], 'list-after-0034');
assert.ok(pendingOutput.includes('No migrations to apply'), 'Local 0034 remains pending');
...
try {
  ...
  for (const [name, before] of Object.entries(preflight.snapshot.tables)) {
    ...
    if (name === 'd1_migrations') {
      ...
      // Wrangler records migration 35 in d1_migrations, so that one internal
  ...
  assert.equal(sequenceBefore.find(row => row.name === 'd1_migrations').seq, 34);
  assert.equal(sequenceAfter.find(row => row.name === 'd1_migrations').seq, 35);
  ...
  assert.equal((await local.env.DB.prepare('SELECT COUNT(*) AS count FROM result_card_publication_logs').first()).count, 0);
...
const evidence = {
  ...
  sqlite_sequence_change: { table: 'd1_migrations', before: 34, after: 35 },
  migration_history_count: history.length,
  migration_once_and_last: true,
  pending_after: [],
  foreign_key_check: after.foreignKeys,
  publication_counts_after_backfill: publicationCounts,
  readiness,
