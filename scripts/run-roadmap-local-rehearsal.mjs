// Isolated Local D1 rehearsal for the Phases 21E–22B STAGING export. No remote mode exists.
...
for (const path of [backupPath, preflightPath, evidencePath]) assert.ok(relative(root, path).startsWith('..'), 'Artifacts must remain outside the repository');
const migrationNames = ['0042_parent_communication.sql', '0043_grade_progress_reports.sql', '0044_admission_regulations.sql', '0045_admissions_transfers.sql'];
const workRoot = mkdtempSync(join(tmpdir(), 'smart-school-roadmap-staging-local-'));
...
const databaseName = 'smart-school-roadmap-rehearsal-local-only';
const wranglerPath = join(root, 'node_modules', 'wrangler', 'bin', 'wrangler.js');
mkdirSync(migrationsDirectory);
for (const name of migrationNames) copyFileSync(join(root, 'migrations', name), join(migrationsDirectory, name));
...
assert.deepEqual(preflight.pending, migrationNames);
...
runLocal(['migrations', 'apply', databaseName], 'apply-0042-through-0045');
const pendingOutput = runLocal(['migrations', 'list', databaseName], 'list-after-0045');
assert.ok(pendingOutput.includes('No migrations to apply'), 'Local roadmap migrations remain pending');
local = await openLocal();
...
try {
  ...
  for (const oldObject of preflight.snapshot.schema) assert.deepEqual(after.schema.find(item => item.type === oldObject.type && item.name === oldObject.name), oldObject, `Historical schema changed: ${oldObject.name}`);
  ...
  for (const [name, before] of Object.entries(preflight.snapshot.tables)) {
    ...
    if (name === 'd1_migrations') {
      assert.equal(projected.count, before.count + 4);
  ...
  for (const name of migrationNames) assert.equal(history.filter(row => row.name === name).length, 1);
  assert.deepEqual(history.slice(-4).map(row => row.name), migrationNames);
  ...
  assert.equal(Object.keys(after.tables).length, 95);
  for (const name of ['parent_conversations','parent_messages','parent_conversation_reads','parent_conversation_audit','communication_write_guards','grade_progress_reports','grade_progress_audit','workflow_write_guards','admission_regulations','admission_regulation_audit','admission_applications','admission_application_audit']) assert.ok(after.tables[name], `Missing ${name}`);
  readiness = {};
...
assert.equal(evidence.application_table_count, 93);
