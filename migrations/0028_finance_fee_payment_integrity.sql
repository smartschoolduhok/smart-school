...
-- Remote-parser compatibility: conditional guards use SELECT RAISE ... WHERE.
-- Enclose calculated CASE values: only the trigger itself may end with END;.
...
WHEN OLD.currency='IQD' BEGIN
  ...
  WHERE (SELECT healthy FROM finance_fee_readiness WHERE id=OLD.id)!=1
...
CREATE TRIGGER trg_student_fees_finance_insert BEFORE INSERT ON student_fees BEGIN
  ...
  WHERE NEW.net_fee IS NULL OR NEW.discount_amount IS NULL OR NEW.discount_type IS NULL OR NEW.discount_value IS NULL
  ...
  WHERE NEW.amount<=0 OR NEW.amount>9007199254740991 OR NEW.amount!=CAST(NEW.amount AS INTEGER)
  ...
  WHERE NEW.fee_type_key IS NULL OR NEW.fee_type_key!=NEW.fee_type
  ...
  WHERE NOT EXISTS(SELECT 1 FROM students s JOIN schools sch ON sch.id=s.school_id WHERE s.id=NEW.student_id AND s.school_id=NEW.school_id AND s.status='active' AND sch.status='active')
  ...
  SELECT RAISE(ABORT,'invalid_finance_request')
  WHERE NEW.status!= ( CASE WHEN NEW.net_fee=0 THEN 'paid' ELSE 'pending' END );
END;
...
CREATE TRIGGER trg_student_fees_finance_update BEFORE UPDATE ON student_fees BEGIN
  ...
  WHERE NEW.net_fee IS NULL OR NEW.discount_amount IS NULL OR NEW.discount_type IS NULL OR NEW.discount_value IS NULL
    ...
    OR (NEW.discount_type='percentage' AND (NEW.discount_value>100 OR round(NEW.discount_value,2)!=NEW.discount_value
      OR NEW.discount_amount!=(CAST(NEW.amount AS INTEGER)/10000)*CAST(round(NEW.discount_value*100) AS INTEGER)
        +((CAST(NEW.amount AS INTEGER)%10000)*CAST(round(NEW.discount_value*100) AS INTEGER)+5000)/10000));
  SELECT RAISE(ABORT,'payment_treasury_integrity_error')
  WHERE EXISTS(SELECT 1 FROM fee_payments WHERE student_fee_id=OLD.id AND
  ...
  WHERE NEW.school_id!=OLD.school_id OR NEW.student_id!=OLD.student_id OR NEW.academic_year_id IS NOT OLD.academic_year_id;
  SELECT RAISE(ABORT,'unsupported_finance_currency')
  WHERE NEW.currency!='IQD' OR OLD.currency!='IQD';
  SELECT RAISE(ABORT,'invalid_finance_amount')
  WHERE NEW.amount<=0 OR NEW.amount>9007199254740991 OR NEW.amount!=CAST(NEW.amount AS INTEGER)
  ...
  SELECT RAISE(ABORT,'fee_net_below_paid')
  WHERE NEW.net_fee<(SELECT coalesce(SUM(CAST(amount AS INTEGER)),0) FROM fee_payments WHERE school_id=NEW.school_id AND student_fee_id=NEW.id AND status='active');
  SELECT RAISE(ABORT,'finance_reconciliation_required')
  WHERE NEW.paid_amount!=(SELECT coalesce(SUM(CAST(amount AS INTEGER)),0) FROM fee_payments WHERE school_id=NEW.school_id AND student_fee_id=NEW.id AND status='active')
    OR NEW.status!= ( CASE WHEN NEW.paid_amount>=NEW.net_fee THEN 'paid' WHEN NEW.paid_amount>0 THEN 'partial' ELSE 'pending' END );
  SELECT RAISE(ABORT,'invalid_finance_request')
  WHERE NEW.fee_type_key IS NULL
...
CREATE TRIGGER trg_fee_payments_finance_insert BEFORE INSERT ON fee_payments BEGIN
  SELECT RAISE(ABORT,'unsupported_finance_currency')
  WHERE EXISTS(SELECT 1 FROM treasury_transactions WHERE school_id=NEW.school_id AND status='active' AND currency!='IQD');
  SELECT RAISE(ABORT,'payment_treasury_integrity_error')
  WHERE EXISTS(SELECT 1 FROM fee_payments WHERE student_fee_id=NEW.student_fee_id AND
  ...
  SELECT RAISE(ABORT,'invalid_finance_request')
  WHERE NEW.status!='active' OR NEW.cancelled_at IS NOT NULL OR NEW.cancel_reason IS NOT NULL
  ...
  WHERE NEW.amount<=0 OR NEW.amount>9007199254740991 OR NEW.amount!=CAST(NEW.amount AS INTEGER);
  SELECT RAISE(ABORT,'finance_not_found')
  WHERE NOT EXISTS(SELECT 1 FROM student_fees f JOIN students s ON s.id=f.student_id AND s.school_id=f.school_id JOIN schools sch ON sch.id=f.school_id
  ...
  SELECT RAISE(ABORT,'unsupported_finance_currency')
  WHERE (SELECT currency FROM student_fees WHERE id=NEW.student_fee_id)!='IQD';
  SELECT RAISE(ABORT,'finance_reconciliation_required')
  WHERE (SELECT healthy FROM finance_fee_readiness WHERE id=NEW.student_fee_id)!=1
    OR (SELECT healthy FROM finance_treasury_readiness WHERE school_id=NEW.school_id)!=1;
  SELECT RAISE(ABORT,'fee_not_payable')
  WHERE (SELECT coalesce(net_fee,amount) FROM student_fees WHERE id=NEW.student_fee_id)<=0;
  -- Runs against current DB state INSIDE the writer transaction, not a JS snapshot.
  SELECT RAISE(ABORT,'payment_overpay')
  WHERE NEW.amount+(SELECT coalesce(SUM(CAST(amount AS INTEGER)),0) FROM fee_payments WHERE student_fee_id=NEW.student_fee_id AND school_id=NEW.school_id AND status='active')
...
CREATE TRIGGER trg_fee_payments_post AFTER INSERT ON fee_payments BEGIN
  ...
  SELECT RAISE(ABORT,'invalid_finance_amount')
  WHERE abs((SELECT coalesce(SUM( CASE WHEN transaction_type='income' THEN CAST(amount AS INTEGER) ELSE -CAST(amount AS INTEGER) END ),0) FROM treasury_transactions WHERE school_id=NEW.school_id AND status='active'))>9007199254740991
...
CREATE TRIGGER trg_fee_payments_finance_update BEFORE UPDATE ON fee_payments BEGIN
  SELECT RAISE(ABORT,'unsupported_finance_currency')
  WHERE EXISTS(SELECT 1 FROM treasury_transactions WHERE school_id=OLD.school_id AND status='active' AND currency!='IQD');
  SELECT RAISE(ABORT,'finance_operation_stale')
  WHERE NEW.id!=OLD.id OR NEW.school_id!=OLD.school_id OR NEW.student_id!=OLD.student_id OR NEW.student_fee_id!=OLD.student_fee_id
  ...
  SELECT RAISE(ABORT,'invalid_finance_request')
  WHERE NEW.cancelled_at IS NULL OR NEW.cancelled_at<=0 OR NEW.cancelled_by_user_id IS NULL OR length(trim(coalesce(NEW.cancel_reason,''))) NOT BETWEEN 1 AND 1000;
  SELECT RAISE(ABORT,'payment_receipt_active')
  WHERE EXISTS(SELECT 1 FROM fee_receipt_payments WHERE payment_id=OLD.id AND is_active=1);
  SELECT RAISE(ABORT,'payment_treasury_integrity_error')
  WHERE NOT EXISTS(SELECT 1 FROM treasury_transactions t JOIN student_fees f ON f.id=OLD.student_fee_id AND f.school_id=OLD.school_id AND f.student_id=OLD.student_id
  ...
  SELECT RAISE(ABORT,'finance_reconciliation_required')
  WHERE (SELECT healthy FROM finance_fee_readiness WHERE id=OLD.student_fee_id)!=1
    OR (SELECT healthy FROM finance_treasury_readiness WHERE school_id=OLD.school_id)!=1;
END;
...
CREATE TRIGGER trg_fee_receipts_validate_insert BEFORE INSERT ON fee_receipts BEGIN
  ...
  WHERE NEW.status!='active' OR NOT json_valid(NEW.payment_ids_json);
  SELECT RAISE(ABORT,'receipt_payment_invalid')
  WHERE json_type(NEW.payment_ids_json)!='array' OR json_array_length(NEW.payment_ids_json) NOT BETWEEN 1 AND 100
  ...
  SELECT RAISE(ABORT,'receipt_payment_invalid')
  WHERE EXISTS(SELECT 1 FROM json_each(NEW.payment_ids_json) j LEFT JOIN fee_payments p ON p.id=j.value AND p.school_id=NEW.school_id AND p.student_id=NEW.student_id
  ...
  SELECT RAISE(ABORT,'receipt_academic_year_conflict')
  WHERE (SELECT COUNT(DISTINCT coalesce(f.academic_year_id,0)) FROM json_each(NEW.payment_ids_json) j
  ...
  SELECT RAISE(ABORT,'receipt_academic_year_conflict')
  WHERE NEW.academic_year_snapshot IS NOT (SELECT y.name FROM json_each(NEW.payment_ids_json) j
  ...
  SELECT RAISE(ABORT,'invalid_finance_amount')
  WHERE NEW.total_amount!=(SELECT SUM(p.amount) FROM fee_payments p JOIN json_each(NEW.payment_ids_json) j ON j.value=p.id WHERE p.school_id=NEW.school_id AND p.student_id=NEW.student_id)
    OR NEW.total_amount>9007199254740991;
END;
...
CREATE TRIGGER trg_fee_receipts_immutable BEFORE UPDATE ON fee_receipts BEGIN
  SELECT RAISE(ABORT,'finance_operation_stale')
  WHERE NEW.id!=OLD.id OR NEW.school_id!=OLD.school_id OR NEW.student_id!=OLD.student_id OR NEW.receipt_number IS NOT OLD.receipt_number
  ...
  SELECT RAISE(ABORT,'invalid_finance_request')
  WHERE NEW.status='cancelled' AND (NEW.cancelled_at IS NULL OR NEW.cancelled_at<=0 OR NEW.cancelled_by_user_id IS NULL OR length(trim(coalesce(NEW.cancel_reason,''))) NOT BETWEEN 1 AND 1000);
  SELECT RAISE(ABORT,'invalid_finance_request')
  WHERE NEW.status='active' AND (NEW.cancelled_at IS NOT OLD.cancelled_at OR NEW.cancelled_by_user_id IS NOT OLD.cancelled_by_user_id OR NEW.cancel_reason IS NOT OLD.cancel_reason);
END;
...
CREATE TRIGGER trg_fee_receipt_links_insert BEFORE INSERT ON fee_receipt_payments BEGIN
  SELECT RAISE(ABORT,'receipt_payment_invalid')
  WHERE NOT EXISTS(SELECT 1 FROM fee_receipts r JOIN fee_payments p ON p.id=NEW.payment_id AND p.school_id=r.school_id AND p.student_id=r.student_id
...
CREATE TRIGGER trg_fee_receipt_links_update BEFORE UPDATE ON fee_receipt_payments BEGIN
  SELECT RAISE(ABORT,'finance_operation_stale')
  WHERE NEW.receipt_id!=OLD.receipt_id OR NEW.payment_id!=OLD.payment_id OR NEW.school_id!=OLD.school_id OR NEW.is_active!=0 OR OLD.is_active!=1
...
CREATE TRIGGER trg_fee_receipts_print_guard BEFORE INSERT ON print_records WHEN NEW.source_type='fee_receipts' BEGIN
  SELECT RAISE(ABORT,'receipt_already_cancelled')
  WHERE NOT EXISTS(SELECT 1 FROM fee_receipts WHERE id=NEW.source_id AND school_id=NEW.school_id AND status='active');
END;
