...
Phase 20D.1 مكتملة على فرع `codex/phase-20d-result-publication` وفي PR `#43` الجاهز للمراجعة دون دمج تلقائي. تضيف دورة `draft → published → withdrawn` لكروت النتائج، وتجعل المنشور فقط ظاهرًا لولي الأمر وقابلًا للتحقق والطباعة. نجحت Quality Gates وCloudflare Branch Preview والنسخة الكاملة وpreflight والاستعادة المحلية وSTAGING QA المصادق عليه. طُبّقت migration `0034` وحدها على STAGING؛ لم يُمس Production.
...
يوجد على فرع Phase 20D.1 عدد 35 ملف migration حتى `0034`. على `main` توجد 34 migration حتى `0033`، وعلى STAGING توجد 35 migration مطبقة حتى `0034` مرة واحدة وبالترتيب ولا توجد migrations معلّقة. المصدر المعتمد للترتيب هو أسماء الملفات داخل `migrations/`، بما فيها ملفا `0014` التاريخيان.
...
Phase 20B وPhase 20C مقبولتان ومندمجتان، وmigrations `0032` و`0033` مطبقتان على STAGING. Phase 20D.1 اجتازت الاختبارات والبوابات والنسخ والاستعادة وSTAGING QA؛ migration `0034` مطبقة مرة واحدة وأخيرة وPR `#43` جاهز للمراجعة فقط. بعدها تكون Phase 20D.2 لربط النتيجة الرسمية المنشورة بالترحيل/الإعادة/التخرج. لا تصل إلى Production قبل قرار GO مستقل.
