...
Phase 20D.1 قيد التسليم على فرع مستقل. تضيف دورة `draft → published → withdrawn` لكروت النتائج، وتجعل المنشور فقط ظاهرًا لولي الأمر وقابلًا للتحقق والطباعة. المحاكاة المحلية ناجحة؛ migration `0034` لم تطبق على STAGING أو Production بعد.
...
يوجد على فرع Phase 20D.1 عدد 35 ملف migration حتى `0034`. على `main` وSTAGING توجد 34 migration مطبقة حتى `0033` مرة واحدة وبالترتيب. المصدر المعتمد للترتيب هو أسماء الملفات داخل `migrations/`، بما فيها ملفا `0014` التاريخيان.
...
Phase 20B وPhase 20C مقبولتان ومندمجتان، وmigrations `0032` و`0033` مطبقتان على STAGING. Phase 20D.1 اجتازت الاختبارات المحلية وتنتظر PR وbackup/preflight وSTAGING QA قبل الدمج. بعدها تكون Phase 20D.2 لربط النتيجة الرسمية المنشورة بالترحيل/الإعادة/التخرج. لا تصل إلى Production قبل قرار GO مستقل.
