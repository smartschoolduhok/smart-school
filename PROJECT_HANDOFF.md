...
Phase 20A1 ودفعة ما بعد التدقيق وPhase 20B مندمجة في `main`. migrations `0029`–`0032` مطبقة على STAGING دون Production، ولا توجد migrations معلّقة هناك. Phase 20B أغلقت حساب الطالب المالي وخطط التقسيط الاختيارية وإيصال A4 وعرض ولي الأمر للقراءة فقط بعد backup وpreflight واستعادة محلية وQA وظيفي ناجح.

Phase 20C قيد المراجعة على فرع مستقل. تضيف سياسة سنوية versioned لكل صف لحساب النجاح والإكمال والرسوب والإعفاء والدخول الوزاري ودرجات القرار دون تثبيت قرار سنة بعينها في الكود. التحقق المحلي ناجح؛ migration `0033` لم تطبق بعد على STAGING أو Production.
...
يوجد على فرع Phase 20C عدد 34 ملف migration حتى `0033`. على `main` وSTAGING توجد حاليًا الملفات المطبقة حتى `0032`. المصدر المعتمد للترتيب هو أسماء الملفات داخل `migrations/`، بما فيها ملفا `0014` التاريخيان.
...
Phase 20B مقبولة ومندمجة، وmigration `0032` مطبقة على STAGING. Phase 20C اجتازت البوابات المحلية وتنتظر backup/preflight وتطبيق `0033` وQA على STAGING قبل قبول الدمج. لا تصل إلى Production قبل قرار GO مستقل.
